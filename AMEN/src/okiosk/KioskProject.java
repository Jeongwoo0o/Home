...
import chart.MyFrame;
import model.ordermodel;
import model.VO.BurgerVO;
import view.BurgerView;
...
public class KioskProject extends JFrame {
	...
	public ArrayList<ArrayList<Object>> result = new ArrayList<>();
	BurgerTableModel tbModelBurger;


	public void addResult() {
		// PanelA에 있는 화면 JTable(View) / 테이블 모델에 지정
		tbModelBurger.setData(result); // 수정된 부분
	...
	void addMenuButtons() {
		...
		bPanel.setLayout(new GridLayout(0, 5));
		bPanel.add(event);
		bPanel.add(set);
		bPanel.add(burger);
		bPanel.add(side);
	...
	class MenuActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand().equals("행사상품")) {
				...
				contentpane.remove(drinkView);
				contentpane.add(homeView, BorderLayout.CENTER);
			} else if (e.getActionCommand().equals("통계")) {
				new MyFrame();
			}
	...
	void addBottomPanel() {
		...
		b4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int rowCount = tbModelBurger.getRowCount();
				if (rowCount > 0) {
					tbModelBurger.data.clear();
					tbModelBurger.fireTableDataChanged();
				} else {
					JOptionPane.showMessageDialog(null, "삭제할 메뉴가 없습니다.");
				}
			}
		});
		...
		b3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (option == JOptionPane.YES_OPTION) {
					...
					try {
						for(ArrayList<Object> rowData : tbModelBurger.data) {
							BurgerVO vo = new BurgerVO();
							vo.setpName((String) rowData.get(0));
							vo.settAmount((int) rowData.get(1));
							vo.setPrice((int) rowData.get(2));
							
							model.insertorder(vo);
					...
					JOptionPane.showMessageDialog(null, "결제 취소");
				}
			}
		});
	...
	public void checkCoupon() {
		String cNum = JOptionPane.showInputDialog("쿠폰번호 입력");
		try {
			ArrayList list = model.checkCoupon(cNum);
			if(list.get(0) == "20240402" && list.get(1) == "N" && list.get(2) == "20240405") {
